...
# Local Dev testing
load_dotenv()
nc_test_email = os.getenv(
    "NC_SALES_TAX_CONTACT_EMAIL"
)  # this number wont change in prod so no need to collect it in the gui
nc_test_login_name = os.getenv(
    "NC_SALES_TAX_CONTACT_NAME"
)  # this number wont change in prod so no need to collect it in the gui
nc_test_phone = os.getenv(
    "NC_SALES_TAX_CONTACT_PHONE"
)  # this number wont change in prod so no need to collect it in the gui
nc_test_filing_url = os.getenv(
    "NC_SALES_TAX_FILING_URL"
)  # this number wont change in prod so no need to collect it in the gui
nc_test_account_id = os.getenv(
    "NC_SALES_TAX_ACCOUNT_ID"
)  # this number wont change in prod so no need to collect it in the gui
nc_test_ein = os.getenv("BARKINGLABS_EIN")
nc_test_period = "Apr 2026"
nc_test_template = "/Users/riley/Downloads/NC_TAX_TEMPLATE_2026.04.csv"
...
def NC_file_tax(
    contact_name, contact_email, contact_phone, account_id, period, ein, file_path
):
    ...
    file_type_selector = driver.find_element(
        By.CSS_SELECTOR, "input[name='filingOption'][value='filePay']"
    )
    ...
    fein_type_selector = driver.find_element(
        By.CSS_SELECTOR, "input[name='entityType'][value='001']"
    )
    ...
    for _, row in tax_data.iterrows():
        ...
        try:
            ...
            if county_juris == "GROSS TOTAL":
                ...
                set_value_js_by_name(
                    driver, "receipts-county-0225", f"{high_local_tax}"
                )
    ...
    next_button = driver.find_element(By.NAME, "img_e536")
    next_button.click()
    ...
    for _, row in tax_data.iterrows():
        ...
        try:
            # Find input index
            if (
                county_juris == "GROSS TOTAL"
                or county_juris == "NON TAXABLE TOTAL"
                or county_juris == "GROSS TOTAL"
            ):
                pass
